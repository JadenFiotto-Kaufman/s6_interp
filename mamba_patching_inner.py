...
with model.forward(validate=False) as generator:
    ...
    with generator.invoke(clean_prompt, scan=False) as invoker:
        ...
        # ]
        clean_hs = [
            torch.concatenate([model.backbone.layers[layer_idx].mixer.ssm.hx.output, *[model.backbone.layers[layer_idx].mixer.ssm.hx.next().output for i in range(len(clean_tokens) - 1)]])
            for layer_idx in range(len(model.backbone.layers))
        ]
    ...
    for layer_idx in range(len(model.backbone.layers)):
        ...
        for token_idx in range(len(clean_tokens)):
            with generator.invoke(corrupted_prompt, scan=False) as invoker:
                ...
                model.backbone.layers[layer_idx].mixer.ssm.hx.next(token_idx)

                model.backbone.layers[layer_idx].mixer.ssm.hx.output[:] = clean_hs[
                    layer_idx
                ][token_idx]
...
fig.write_image("Ctpatching.png")
